import {
	InverterSettingsDto,
	InverterStatus,
} from '../dto/inverter-settings.dto';
import { InverterModel } from '../../types';
import { localize } from '../../localize/localize';

export class SRNE extends InverterSettingsDto {
	brand = InverterModel.SRNE;
	image =
		'data:image/png;base64,';
	statusGroups: InverterStatus = {
		standby: {
			states: ['standby state', 'manual shutdown'],
			color: 'blue',
			message: localize('common.standby'),
		},
		ongrid: {
			states: ['ac power operation', 'ac power to inverter'],
			color: 'green',
			message: localize('common.ongrid'),
		},
		offgrid: {
			states: [
				'inverter operation',
				'inverter to ac power',
				'battery activation',
			],
			color: 'green',
			message: localize('common.offgrid'),
		},
		fault: {
			states: ['fault'],
			color: 'red',
			message: localize('common.fault'),
		},
		check: {
			states: ['initialization', 'power-on delay', 'soft start'],
			color: 'yellow',
			message: localize('common.selftest'),
		},
	};
}
